...
const Page = () => {
  const [categories] = useState(Array(10).fill({ name: 'Scale', id: '234454545' }));
  const [currentPage, setCurrentPage] = useState(1); // Current page set to 2 based on your image
      ...
      <div className="flex flex-col sm:flex-row min-h-screen bg-[#121212] text-white">
        ...
        <div className="bg-[#1b1b1b] w-full sm:w-1/5 p-6">
          <ul className="space-y-4">
            ...
            <li className="">
              <Link href="/SubCategories" className="flex items-center text-lg hover:bg-[#2b2b2b] p-2 rounded-md">
              ...
              {[1, 2, 3].map((page) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-full ${
                    page === currentPage ? 'bg-[#fd4960]' : 'bg-[#2b2b2b]'
                  }`}
                >
                  {page}
                </button>
              ))}
              <button className="px-2 py-2 rounded-full bg-[#2b2b2b] hover:bg-[#3b3b3b]"><IoIosArrowForward /></button>
            </div>
          </div>
        </div>
      </div>
